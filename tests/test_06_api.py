...
def client(tmp_path, monkeypatch):
    """Fresh app instance with isolated storage so quota/users don't leak across runs."""
    ...
    monkeypatch.delenv("SYNCFORGE_REQUIRE_AUTH", raising=False)
...
def test_create_job_returns_id(client, monkeypatch):
    ...
    async def fake_run(jid, req):
        ...
        await main.publish(jid, {"event": "done", "result": {"ok": True}})
...
def test_websocket_replays_events(client, monkeypatch):
    import app.main as main
    jid = "wsjob123"
    main.JOBS[jid] = main.Job(id=jid, status="done",
                    events=[{"event": "start"},
                            {"event": "step", "step": "tts"},
